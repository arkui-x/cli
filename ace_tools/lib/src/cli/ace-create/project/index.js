...
function createProject(projectPath, bundleName, project, runtimeOS, proType, template, currentProjectPath) {
  try {
    ...
    if (proType === aceLibrary) {
      if (!(createAar(projectPath, "library") && createFramework(projectPath, "myframework"))) {
        return false;
      }
    }
    if (proType !== aceLibrary) {
      console.log(`
Project created. Target directory:  ${projectPath}.
...
function replaceStageProjectInfo(projectPath, bundleName, project, runtimeOS, proType, template) {
  ...
  const strs = [];
  if (proType === aceLibrary) {
    files.push(path.join(projectPath, 'hvigorfile.ts'));
